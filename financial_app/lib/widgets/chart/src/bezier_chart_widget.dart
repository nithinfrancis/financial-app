...
class BezierChartState extends State<BezierChart> with SingleTickerProviderStateMixin {
  ...
  double _buildContentWidth(BoxConstraints constraints) {
    ...
    if (scale == BezierChartScale.CUSTOM) {
      ...
      if (scale == BezierChartScale.HOURLY) {
        ...
        return (_xAxisDataPoints.length * (horizontalSpacing * _currentScale) - horizontalPadding / 2) * 2;
      } else if (scale == BezierChartScale.YEARLY) {
  ...
  _onLayoutDone(_) {
    ...
    if ((widget.selectedDate != null && _currentBezierChartScale != BezierChartScale.CUSTOM) || (widget.selectedValue != null && _currentBezierChartScale == BezierChartScale.CUSTOM)) {
      ...
      if (_currentBezierChartScale == BezierChartScale.WEEKLY) {
        ...
        index = _xAxisDataPoints.indexWhere((dp) => (dp.xAxis as DateTime).year == widget.selectedDate!.year && (dp.xAxis as DateTime).month == widget.selectedDate!.month && (dp.xAxis as DateTime).day == widget.selectedDate!.day && (dp.xAxis as DateTime).hour == widget.selectedDate!.hour);
  ...
  _computeSeries() {
    ...
    if (_currentBezierChartScale == BezierChartScale.MONTHLY || _currentBezierChartScale == BezierChartScale.YEARLY || _currentBezierChartScale == BezierChartScale.WEEKLY || _currentBezierChartScale == BezierChartScale.HOURLY) {
      for (BezierLine line in widget.series) {
        ...
        child: GestureDetector(
          ...
          child: LayoutBuilder(
            builder: (context, constraints) {
              ...
              final items = <Widget>[];
              maxHeight = constraints.biggest.height * 0.80;
              items.add(
                                  ...
                                  return Container(
                                    ...
                                    child: Column(
                                      ...
                                      children: [
                                        Container(
                                          height: maxHeight,
                                          decoration: BoxDecoration(borderRadius: BorderRadius.circular(5.0), color: ((index + 1) == selectedMonth) ? const Color(0xffE1E4FC) : const Color(0xffF5F6FA)),
                                        ),
                                        SizedBox(
                                          height: 20,
                                        ),
                                        Text(intl.DateFormat('MMM').format(DateTime(0, index + 1)).toUpperCase(),
                                        style: TextStyle(color: AppColors.greyColor),
                                        ),
                                      ],
                                    ),
                                    width: ((_contentWidth + 50) / 12),
                                  );
                                }),
                          ),
                          Padding(
              ...
              if (widget.config!.displayYAxis) {
                ...
                _addYItem(double value, {Key? key}) {
                  items.add(
                    Positioned(
                      bottom: _getRealValue(value - (widget.config!.startYAxisFromNonZeroValue ? _yValues!.first! : 0.0), maxHeight - widget.config!.footerHeight, maxValue) + widget.config!.footerHeight + fontSize / 2,
                      left: 10.0,
                      child: Text(
...
class _BezierChartPainter extends CustomPainter {
  ...
  @override
  bool shouldRepaint(_BezierChartPainter oldDelegate) => shouldRepaintChart! || oldDelegate.verticalIndicatorPosition != verticalIndicatorPosition || oldDelegate.scrollOffset != scrollOffset || oldDelegate.showIndicator != showIndicator;
}
...
bool areEqualDatesIncludingHour(DateTime dateTime1, DateTime dateTime2) => dateTime1.year == dateTime2.year && dateTime1.month == dateTime2.month && dateTime1.day == dateTime2.day && dateTime1.hour == dateTime2.hour;
